...
import { mockBooks, categories, BookCondition } from '@/data/mockBooks';
...
export default function Browse() {
  ...
  const filteredBooks = useMemo(() => {
    ...
    return result;
  }, [searchQuery, selectedCategory, selectedCondition, selectedPriceRange, sortBy]);
    ...
    <Layout>
      ...
      <div className="bg-gradient-to-b from-sage-light/30 to-background py-12">
        <div className="container-booksnap">
          <h1 className="text-3xl lg:text-4xl font-serif font-bold text-foreground text-center">
            Browse Books
          </h1>
                ...
                <div className="space-y-2">
                  {conditions.map((condition) => (
                    <button
                      key={condition}
                      onClick={() =>
                        updateFilter('condition', selectedCondition === condition ? '' : condition)
                      }
                      className={cn(
                ...
                <div className="space-y-2">
                  {priceRanges.map((range) => (
                    <button
                      key={range.label}
                      onClick={() =>
                        updateFilter('price', selectedPriceRange === range.label ? '' : range.label)
                      }
                      className={cn(
                        'w-full px-3 py-2 rounded-lg text-sm text-left transition-colors',
